...
Typing a filename from memory is a guess. Letting the shell complete it is not
— it is spelled right or it does not exist. This is worth repeating because
it is the single most common thing people forget under time pressure.
:::
...
The shell splits what you type on whitespace — so a space inside a filename
is indistinguishable from a space between two separate arguments.
...
`cd` saw **two** arguments, not one. Quoting (`cd "lab notebook"`) fixes it,
but the real fix is upstream: use `lab_notebook` and the problem never comes
up. Graphical file managers and Windows both allow spaces freely — the
terminal is where that difference first bites.
...
a short section on `$HOME`-style variables, and a checklist to test yourself
against.
...
**You will need an account.** At least one person per team, for the shared
repository to exist — in practice, everyone, since each of you pushes your
own commits.

Treat it as more than a requirement: pinned repositories, a short bio, a
clear README on each project — this becomes a portfolio you did not realise
you were building. One real example:
**[github.com/jonas-fuchs](https://github.com/jonas-fuchs)**.

For a job in this field, that can say more than a CV — it is evidence you can
do the work, not a claim that you can.

::: {.notes}
Worth slowing down on if presenting live — this is a genuinely useful thing
for a UAS bioinformatics cohort to hear early, not a footnote. It does not
need to happen this semester; it needs to *start*.
:::
...
## Getting a terminal, cheapest first {.smaller}

Reading the commands is not the same as running them, so the first job is
somewhere to type. Any of these will do.

| | |
|---|---|
| **Nothing to install** | [webvm.io](https://webvm.io) boots a real Debian Linux in a browser tab — no account, nothing downloaded. Enough for all of Part 1. |
| **On a Mac** | You already have one: *Applications → Utilities → Terminal*. |
| **On Windows** | Install **WSL2**: `wsl --install` in PowerShell as administrator, then reboot. It is a real Ubuntu, and everything here then applies to you unchanged. |
| **Getting serious** | Install Linux on a machine of your own. Nothing here requires it; everything here is easier once you have. |

The browser one forgets everything when you close the tab and is not the place
to install software — so Part 2 onward wants one of the other three.

::: {.notes}
The point of the table is that nobody is blocked. The commonest reason someone
arrives having read nothing is that step one was an install they did not have
time for -- so the first row exists to remove that excuse entirely.
...
Worth saying out loud: some courses hand you a prepared machine, and if yours
does, none of this is required of you.
:::

## What to actually do this week

- **Get a terminal**, by whichever route above costs you least.
- Work through **[Part 1](parts/01-command-line.qmd)** properly — *typing* the
  commands — and check yourself against its list at the end.
...
If your course provides a prepared machine, none of the setup is required of
you. The hour spent typing into a terminal of your own is still what makes the
prepared one feel ordinary rather than magic.
